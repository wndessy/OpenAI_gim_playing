# -*- coding: utf-8 -*-
"""
Created on Fri Nov  9 12:51:13 2018
...
def find_parents(ann_rewards):
    """
    Calculate the probabilities of ANNs and find two parents.
    Find best and average score.
    """
    best_score = max(ann_rewards)
    average_score = ann_rewards.mean()
    ann_prob = ann_rewards / sum(ann_rewards)
    parents_idx = np.argsort(ann_prob)[-2:]
    parent1 = ann_population[parents_idx[0]]
    parent2 = ann_population[parents_idx[1]]
    return parent1, parent2, best_score, average_score
...
population_size = 10  # Between 10 and 100, depending on CPU
generations = 20  # Between 10 and 20
...
env._max_episode_steps=200  # np.inf , for infinity time
...
ann_population = initial_population(population_size)
...
for pop in range(generations):
    ...
    rewards_summary[pop] = ann_rewards

    # Find two parents based on rewards
    parent1, parent2, best, average = find_parents(ann_rewards)
    ...
    # Breeding 1: From only two chosen parents create the whole new population
